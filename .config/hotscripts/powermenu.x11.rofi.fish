...
### Answer
set chosen (echo -e $options | rofi -theme $powermenu_theme -p 'Goodbye, '$USER -dmenu -selected-row 2)
...
switch $chosen
	...
	case $lock
		betterlockscreen -l dim -t "What are you doing step bro?~"
	case $suspend
		set ans (confirm_exit)
		if test $ans = 'y'
			playerctl pause
			betterlockscreen -s dim -t "What are you doing step bro?~"
		else if test $ans = 'n'
			exit 0
		end
	case $logout
